...
class Connect
{
     ...
     * @return void
     */
    public function addUser($user, $pass, $authority, $params)
    {
        $stmt = $this->db->prepare("INSERT into Shop_User (name, pass, authority) VALUES ('$user', '$pass', '$authority')");
        $stmt->execute();

        $id = $this->db->lastInsertId();
        $stmt = $this->db->prepare("UPDATE Shop_User SET firstName = ?, lastName = ? WHERE id = $id");
        $stmt->execute($params);
    }
    ...
    {
        $stmt = $this->db->prepare("SELECT pass FROM Shop_User WHERE name='$user'");
        $stmt->execute();
    ...
    {
        $stmt = $this->db->prepare("SELECT info, email, authority FROM Shop_User WHERE name='$user'");
        $stmt->execute();
    ...
    public function edit($params, $user)
    {
        ...
        $stmt = $this->db->prepare("UPDATE Shop_User SET info = ?, email = ? WHERE name='$user'");
    ...
    {
        $stmt = $this->db->prepare("UPDATE Shop_User SET pass='$pass' WHERE name='$user'");
        $stmt->execute();
    }
    ...
    {
        $stmt = $this->db->prepare("UPDATE Shop_User SET pass='$pass' WHERE id='$id'");
        $stmt->execute();
    }

    public function controlBlock($user)
    {
        $res = $this->getRes("SELECT authority FROM Shop_User WHERE name='$user'");

        if ($res["authority"] != "Blocked") {
            return true;
        }
        return false;
    }
    ...
    {
        $stmt = $this->db->prepare("SELECT * FROM Shop_User WHERE name='$user'");
        $stmt->execute();
